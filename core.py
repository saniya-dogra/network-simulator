...
def short_mac(mac):
    return f"{mac[:8]}..." if len(mac) > 8 else mac
...
class Hub(Device): # hub is a netowrking device which inherits from Device 
    ...
    def broadcast(self, sender, frame, datalink_layer): # broadcasting data to all devices
        print(f"\n[HUB:{self.name}] repeat frame from {sender.name} to all other ports")
...
class Switch(Device):
    ...
    def forward(self, sender, frame, datalink_layer):
        print(f"\n[SW:{self.name}] ingress frame from {sender.name}")
        ...
        if frame.source_mac not in self.mac_table:
            ...
            print(f"[SW:{self.name}] learn src {short_mac(frame.source_mac)} -> {sender.name}")
        ...
        if frame.dest_mac in self.mac_table:
            ...
            print(f"[SW:{self.name}] unicast dst {short_mac(frame.dest_mac)} -> {dest_device.name}")
        ...
        # 3. FLOODING: First time seeing this MAC? Send to everyone except sender
            ...
            print(f"[SW:{self.name}] dst {short_mac(frame.dest_mac)} unknown; flooding")
